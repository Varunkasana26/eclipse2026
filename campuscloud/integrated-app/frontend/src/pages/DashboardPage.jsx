...
import { Activity, Box, Cpu, Radio, Server } from 'lucide-react';
  ...
  useEffect(() => {
    return connectToEventStream({
      ...
      onMessage: (message) => {
        ...
        if (event === 'job:submit' || event === 'job:update' || event === 'job:complete' || event === 'job:failed') {
          if (payload?.job) {
            setJobs((current) => upsertById(current, payload.job));
          }
          return;
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (!Array.isArray(command) || command.length === 0) {
        throw new Error('Command must be a JSON array, for example ["node","-e","console.log(1)"]');
      }
        ...
        <section className="bg-slate-950/80 border border-slate-800 rounded-3xl p-6">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
            <div>
              ...
              <h1 className="text-3xl font-bold mt-2">GPU Sharing Demo Console</h1>
              <p className="text-slate-400 mt-3 max-w-2xl">
                Live view of registered provider nodes, queued jobs, assignment state, and streamed execution logs.
              </p>
            </div>
            <div className="flex items-center gap-3 bg-slate-900 border border-slate-800 rounded-2xl px-4 py-3">
              ...
              <span className="text-sm">
                ...
                <span className={`ml-2 font-semibold ${
                  connectionState === 'live' ? 'text-emerald-300' : 'text-amber-300'
                }`}>
                  {connectionState}
                </span>
              </span>
