...
async function attemptRegistration(systemInfo) {
  try {
    ...
    logger.warn("Worker registration failed, retrying once in 10 seconds", {
      worker_id: config.workerId || systemInfo.hostname,
      error: error.message
    });
  }
  ...
  await registerWorker(systemInfo);
  logger.info("Worker registered with backend on retry", {
    worker_id: config.workerId || systemInfo.hostname,
    backend_url: config.backendUrl
  });
}
...
async function main() {
  ...
  if (config.executorMode === "http" && !systemInfo.docker.docker_available) {
    logger.warn("HTTP executor mode selected but Docker is unavailable", {
      worker_id: config.workerId || systemInfo.hostname
    });
  }
